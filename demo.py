...
# from pitchreg.sportsfield_release.utils import warp, utils

sys.path.append('deepeiou/deepeiou/tools')

from loguru import logger

from teamdetect.preprocessing import crop_image, save_img
from teamdetect.preprocessing import preprocess
from teamdetect.clustering import Clustering
from pitchreg.showdiagram import overwrite
from pitchreg.showdiagram import represent_players

from pitchreg.register import run
from pitchreg.register import calculate_pitch_coords
from deepeiou.deepeiou.yolox.data.data_augment import preproc
...
def make_parser():
    ...
    parser.add_argument(
        "--path", default="deepeiou/demo.mp4", help="path to images or video"
    )
    parser.add_argument(
        "--save_result",
        default=True,
    ...
    parser.add_argument(
        "-f",
        "--exp_file",
        default="deepeiou/deepeiou/yolox/yolox_x_ch_sportsmot.py",
        type=str,
    ...
    parser.add_argument("--track_low_thresh", default=0.1, type=float,
                        help="lowest embedding threshold valid for tracks")
...
def running_average_homographies(current_homography, past_homographies, window_size=90):
    past_homographies.append(current_homography)
    if len(past_homographies) > window_size:
        past_homographies.pop(0)
    average_homography = sum(past_homographies) / len(past_homographies)
    return average_homography, past_homographies
    ...
    while True:
        ...
        if ret_val:
            ...
            if outputs[0] is not None:
                ...
                det /= scale
                rows_to_remove = np.any(det[:, 0:4] < 1, axis=1)  # remove edge embedding
                ...
                if cropped_imgs:
                    ...
                    for t in online_targets:
                        ...
                        if tlwh[2] * tlwh[3] > args.min_box_area:
                            ...
                            midpoint = tlwh[0] + tlwh[2] / 2, tlwh[1] + tlwh[3]
                            fov_coords.append([midpoint])
                            ...
                            img = crop_image(frame, tlwh)
                            img_pp = preprocess(img)
                            online_img.append(img)
                            online_data.append(img_pp)
                    if frame_id < 50:
                        teams = cluster.train(online_data)
                    else:
                        teams = cluster.assign(online_data)
                    for id, team in zip(online_ids, teams):
                        if id in player_teams_counts:
                            player_teams_counts[id].append(team)
                            while len(player_teams_counts[id]) > 50:
                                player_teams_counts[id] = player_teams_counts[id][1:]
                        else:
                            player_teams_counts.update({id: [team]})
                    else:
                        timer.toc()
                        ...
                        diagram = represent_players(diagram, real_coords, online_ids, player_teams)
                        online_im = overwrite(online_im, diagram)
                else:
                    timer.toc()
...
def main(exp, args):
    ...
    if not args.trt:
        if args.ckpt is None:
            ckpt_file = "deepeiou/deepeiou/checkpoints/best_ckpt.pth.tar"
        else:
            ckpt_file = args.ckpt
    ...
    extractor = FeatureExtractor(
        ...
        model_path='deepeiou/deepeiou/checkpoints/sports_model.pth.tar-60',
        device='cuda'
    )
